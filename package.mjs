...
export function buildPackages(root, out) {
  ...
  if (manifest.id !== 'port-contract-audit' || !/^\d+\.\d+\.\d+(?:-[a-zA-Z0-9.-]+)?$/.test(manifest.package.version)) throw new Error('Unexpected package identity/version');
  ...
  const stage = mkdtempSync(path.join(tmpdir(),'port-contract-package-'));
